#!/usr/bin/env bash
set -euo pipefail
ROOT=${REBOT_EXPERIMENT_ROOT:-/fsx/pepijn/rebot-fineart-20260925}
...
case "$PHASE" in smoke) STEPS=20; LOG_FREQ=1; WANDB=false; EVAL_STEPS=0 ;; full) STEPS=${REBOT_TRAIN_STEPS:-10000}; LOG_FREQ=20; WANDB=true; EVAL_STEPS=1000 ;; *) exit 2 ;; esac
...
cd "$ROOT/lerobot"
EPISODES=$("$SOURCE/venv/bin/python" -c 'import json,sys; s=json.load(open(sys.argv[1])); print(json.dumps(s["train"]+s["dev"]))' "$ROOT/split.json")
...
exec "$SOURCE/tools/bin/uv" run --no-project --python "$SOURCE/venv/bin/python" \
  ...
  --seed=1000 --env_eval_freq=0 --eval_steps="$EVAL_STEPS" --max_eval_samples="${REBOT_EVAL_SAMPLES:-256}" \
  --save_freq="${REBOT_SAVE_FREQ:-1000}" --log_freq="$LOG_FREQ" \
  --output_dir="$ROOT/runs/${VARIANT}_${PHASE}_${SLURM_JOB_ID:?}" \
  --job_name="fineart_rebot_${VARIANT}_${STEPS}_b64_${PHASE}" \
  ...
  --wandb.notes="${REBOT_TRAIN_NOTES:-Fresh Jade midtrain 300000; KI+FAST; per-GPU16/global64; 85 train/5 dev/10 test; train-only normalization.}"
